$(function () {
    ...
    var jobTable = $("#job_list").dataTable({
        ...
        "serverSide": true,
        "ajax": {
            url: base_url + "/metric-jobs/jobinfo/pageList",
            type: "post",
            data: function (d) {
                var obj = {};
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    return function () {
                        ...
                        // log url
                        var logHref = base_url + '/metric-jobs/joblog?jobId=' + row.id;

                        // code url
                        var codeBtn = "";
                        if ('BEAN' != row.glueType) {
                            var codeUrl = base_url + '/metric-jobs/jobcode?jobId=' + row.id;
    ...
    $('#jobGroup').on('change', function () {
        ...
        window.location.href = base_url + "/metric-jobs/jobinfo?jobGroup=" + jobGroup;
    });
    ...
    $("#job_list").on('click', '.job_operate', function () {
        ...
        if ("job_pause" == type) {
            ...
            url = base_url + "/metric-jobs/jobinfo/stop";
            needFresh = true;
        } else if ("job_resume" == type) {
            typeName = I18n.jobinfo_opt_start;
            url = base_url + "/metric-jobs/jobinfo/start";
            needFresh = true;
        } else if ("job_del" == type) {
            typeName = I18n.system_opt_del;
            url = base_url + "/metric-jobs/jobinfo/remove";
            needFresh = true;
        } else {
            return;
        }
    ...
    $("#jobTriggerModal .ok").on('click', function () {
        $.ajax({
            type: 'POST',
            url: base_url + "/metric-jobs/jobinfo/trigger",
            data: {
    ...
    $("#job_list").on('click', '.job_registryinfo', function () {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "/metric-jobs/jobgroup/loadById",
            data: {
                "id": jobGroup
            },
            dataType: "json",
    ...
    $("#job_list").on('click', '.job_next_time', function () {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + "/metric-jobs/jobinfo/nextTriggerTime",
            data: {
    ...
    var addModalValidate = $("#addModal .form").validate({
        ...
        submitHandler: function (form) {
            ...
            $.post(base_url + "/metric-jobs/jobinfo/add", $("#addModal .form").serialize(), function (data, status) {
                if (data.code == "200") {
                    $('#addModal').modal('hide');
                    layer.open({
                        title: I18n.system_tips,
    ...
    var updateModalValidate = $("#updateModal .form").validate({
        ...
        submitHandler: function (form) {
            ...
            // post
            $.post(base_url + "/metric-jobs/jobinfo/update", $("#updateModal .form").serialize(), function (data, status) {
                if (data.code == "200") {
