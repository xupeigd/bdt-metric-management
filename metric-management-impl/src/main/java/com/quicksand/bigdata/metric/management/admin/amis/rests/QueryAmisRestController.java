 ...
 * @Description: 数据查询控制器
 */
@Tag(name = "数据查询服务", description = "提供数据集和指标数据的查询预览功能")
...
public class QueryAmisRestController {
    ...
    @Operation(description = "数据集数据预览")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "operation success ! "),
            @ApiResponse(responseCode = "404", description = "数据集不存在 !")
    })
    @GetMapping("/datasets/{datasetId}")
    public FrameworkResponse<DynamicTable, Void> datasetExploration(
            @Parameter(description = "数据集ID") @PathVariable("datasetId") Integer datasetId) {
        ...
        DynamicTable dynamicTable = engineService.commonQuery(dataset.getCluster(),
                String.format("SELECT %s FROM %s LIMIT 20", StringUtils.collectionToCommaDelimitedString(fields),
    ...
    public FrameworkResponse<DynamicTable, Void> metricPreview(
            @Parameter(name = "指标Id") @Min(value = 1L, message = "不存在的指标！") @Max(value = Integer.MAX_VALUE, message = "不存在的指标！") @PathVariable("metricId") int metricId) {
        ...
        String completeName = String.format(" %s.%s ", metric.getDataset().getCluster().getDefaultDatabase(),
                metric.getDataset().getTableName());
        ...
        if (AuthUtil.hasAuthority("OP_METRICS_CALCULATE_ACTION")
